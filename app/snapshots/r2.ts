// Server-side R2 access for the snapshots page and the /api/snapshots route.
// Signs S3-compatible requests with SigV4 and reduces the newest completed
// manifest into the `Snapshot` shape the UI renders.
import { createHash, createHmac } from 'node:crypto';

import { unstable_cache } from 'next/cache';

import type { Snapshot, SnapshotComponent } from './data';
import { COMPONENT_META, COMPONENT_ORDER } from './data';
...
/** How long a loaded snapshot list may be reused, for both the page and the API route. */
...
export class SnapshotLoadError extends Error {
  constructor(readonly failures: SnapshotLoadFailure[]) {
    super(failures.map((failure) => `${failure.network}: ${failure.error}`).join('; '));
...
async function loadSnapshots(
  networkId?: string,
): Promise<{ snapshots: Snapshot[]; failures: SnapshotLoadFailure[] }> {
  const configs = networkId ? NETWORKS.filter((config) => config.id === networkId) : NETWORKS;
  const results = await Promise.allSettled(configs.map((config) => getLatestSnapshot(config)));
