...
function DeployModule({ wallet, onSend, onCreated, onNavigate, busy }: { wallet: Address | null; onSend: (label: string, to: Address, data: Hex, action: string) => Promise<Hex | null>; onCreated: (token: RecentToken) => Promise<void>; onNavigate: (module: Module) => void; busy: string | null }) {
...
function CreatedView({ created, onNavigate }: { created: CreatedToken; onNavigate: (module: Module) => void }) {
  const nextSteps: Array<{ module: Module; title: string; body: string }> = [
    ...
    { module: 'memos', title: 'Attach memos', body: 'Send a transfer, mint, or burn with an on-chain bytes32 memo.' },
  ...
  return (
    <div className="flex flex-col gap-5">
      <Text variant="body" tone="muted">Your {created.variant} token {created.symbol} is live on Vibenet and fully initialized. Here’s what was set up, and where to go next.</Text>
      <div className="rounded-xl border border-bds-gray-10 p-4 dark:border-white/10">
      ...
      <div>
        ...
        <ul className="mt-3 space-y-2">{created.configured.map((item) => <li key={item} className="flex items-start gap-2 text-[13px]"><span className="mt-0.5 text-bds-green-60" aria-hidden="true">✓</span><span>{item}</span></li>)}</ul>
        <p className="mt-3 text-[11px] text-bds-gray-50">All applied atomically during initialization, so the token was fully configured in a single transaction.</p>
      </div>
      <div>
        <Text variant="label" tone="muted">What’s next</Text>
        <div className="mt-3 flex flex-col gap-2">{nextSteps.map((step) => <button key={step.module} type="button" onClick={() => onNavigate(step.module)} className="group flex items-center justify-between gap-3 rounded-xl border border-bds-gray-10 p-3 text-left transition-colors hover:border-base-blue dark:border-white/10"><span className="flex min-w-0 flex-col"><Text as="span" variant="label" className="font-medium">{step.title}</Text><Text as="span" variant="footnote" tone="muted" className="mt-0.5">{step.body}</Text></span><span aria-hidden="true" className="shrink-0 text-base-blue transition-transform group-hover:translate-x-0.5">→</span></button>)}</div>
      </div>
    </div>
  );
}
