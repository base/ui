...
function DeployModule({ wallet, onSend, onCreated, onNavigate, busy }: { wallet: Address | null; onSend: (label: string, to: Address, data: Hex, action: string) => Promise<Hex | null>; onCreated: (token: RecentToken) => Promise<void>; onNavigate: (module: Module) => void; busy: string | null }) {
  ...
  if (created) return <CreatedView created={created} onNavigate={onNavigate} onReset={() => setCreated(null)} />;
...
function CreatedView({ created, onNavigate, onReset }: { created: CreatedToken; onNavigate: (module: Module) => void; onReset: () => void }) {
  ...
  return (
    <div className="animate-in flex flex-col gap-5">
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-bds-green-20 bg-bds-green-0 px-6 py-8 text-center dark:border-bds-green-80 dark:bg-bds-green-100/20">
        <span className="flex h-12 w-12 items-center justify-center rounded-full bg-bds-green-50 text-xl text-white" aria-hidden="true">✓</span>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card className="bg-white p-5 dark:bg-white/5">
          ...
          <div className="mt-5 flex flex-wrap gap-4">
            <Link href={`${VIBENET_EXPLORER_PATH}/address/${created.address}`} className="text-[12px] text-base-blue hover:underline">View token on Explorer ↗</Link>
            <Link href={`${VIBENET_EXPLORER_PATH}/tx/${created.hash}`} className="text-[12px] text-base-blue hover:underline">Creation transaction ↗</Link>
          </div>
        </Card>
        <Card className="bg-white p-5 dark:bg-white/5">
