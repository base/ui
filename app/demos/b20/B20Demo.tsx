...
import { cloneElement, isValidElement, useCallback, useEffect, useId, useState, type CSSProperties, type ReactElement } from 'react';
import {
  createPublicClient,
  encodeFunctionData,
  formatEther,
...
import { READ_ANNOUNCEMENT_PROMPT, READ_MEMO_PROMPT, READ_POLICY_PROMPT } from './lib/prompts';
...
function Field({ label, children, hint, help }: { label: string; children: React.ReactNode; hint?: string; help?: string }) {
  const id = useId();
  const hintNode = hint ? <Text as="span" variant="footnote" tone="muted">{hint}</Text> : null;
  // Without help: implicit label association (the control is nested in <label>).
  if (!help) {
    return <label className="flex min-w-0 flex-col gap-1.5"><Text as="span" variant="label" tone="muted">{label}</Text>{children}{hintNode}</label>;
  }
  ...
  return (
    <div className="flex min-w-0 flex-col gap-1.5">
      <div className="flex items-center gap-1.5">
        <label htmlFor={id} className={cn(textVariantClasses.label, 'text-bds-gray-60 dark:text-bds-gray-30')}>{label}</label>
...
export function B20Demo() {
  ...
  useEffect(() => {
    ...
    client.readContract({ address: activeTokenAddress, abi: b20Abi, functionName: 'hasRole', args: [roleId('OPERATOR_ROLE'), wallet] })
  ...
  const send = useCallback(async (label: string, to: Address, data: Hex, action: string): Promise<Hex | null> => {
    const eth = getEthereum();
    if (!wallet || !eth) { setInspectError('Connect a Vibenet wallet first.'); return null; }
...
const INITIAL_ALLOCATION_MEMO = 'Initial deposit';
const INITIAL_ALLOCATION_MAX = 100n;

function MemoModule({ token, tokenAccess, onDeploy, onSend, busy }: { token: TokenInfo | null; tokenAccess: TokenAccess; onDeploy: () => void; onSend: (label: string, to: Address, data: Hex, action: string) => Promise<Hex | null>; busy: string | null }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if ((kind !== 'burn' && !isAddress(to)) || (kind === 'transferFrom' && !isAddress(from))) throw new Error('Enter valid B20 operation addresses.');
      let data: Hex;
      if (kind === 'transfer') data = encodeFunctionData({ abi: b20Abi, functionName: 'transferWithMemo', args: [to as Address, v, m] });
      else if (kind === 'transferFrom') data = encodeFunctionData({ abi: b20Abi, functionName: 'transferFromWithMemo', args: [from as Address, to as Address, v, m] });
      else if (kind === 'mint') data = encodeFunctionData({ abi: b20Abi, functionName: 'mintWithMemo', args: [to as Address, v, m] });
...
function SampleAnnouncementViewer({ onDeploy }: { onDeploy: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <ModuleHeading title="Announcement Reader" description="Read disclosures published for this sample Asset B20." action={<CopyPromptButton prompt={READ_ANNOUNCEMENT_PROMPT} module="announcements" />} />
          ...
          <article key={announcement.id}>
            <Card className="bg-white p-5 dark:bg-white/5">
              ...
              <dl className="mt-5 grid gap-4 border-t border-bds-gray-10 pt-4 text-[13px] md:grid-cols-3 dark:border-white/10">
                ...
                <div><dt className="flex items-center gap-1.5 text-[11px] text-bds-gray-50">Action inside bracket<InfoTooltip label="How announcements work">{B20_HELP.announcementBracket}</InfoTooltip></dt><dd className="mt-1 font-mono text-[11px]">{announcement.call}</dd></div>
              </dl>
...
function AnnouncementModule({ token, tokenAccess, wallet, onDeploy, onSend, busy }: { token: TokenInfo | null; tokenAccess: TokenAccess; wallet: Address | null; onDeploy: () => void; onSend: (label: string, to: Address, data: Hex, action: string) => Promise<Hex | null>; busy: string | null }) {
    ...
    <div className="pointer-events-none absolute inset-x-0 top-5 mx-auto h-24 w-72 overflow-visible" aria-hidden="true">
      {CONFETTI_PIECES.map((piece, index) => (
        <span
          ...
          className={cn('absolute left-1/2 top-8 h-2.5 w-1.5 rounded-full opacity-0 shadow-sm b20-confetti-piece', piece.c, index % 3 === 0 && 'w-4 rounded-sm', index % 3 === 1 && 'h-3')}
          style={{
...
function DeployModule({ wallet, onSend, created, onCreated, onReset, onNavigate, busy }: { wallet: Address | null; onSend: (label: string, to: Address, data: Hex, action: string) => Promise<Hex | null>; created: CreatedToken | null; onCreated: (token: CreatedToken) => Promise<void>; onReset: () => void; onNavigate: (module: Module) => void; busy: string | null }) {
  const [variant, setVariant] = useState<'asset' | 'stablecoin'>('asset'); const [name, setName] = useState('Demo Token'); const [symbol, setSymbol] = useState('DEMO'); const [decimals, setDecimals] = useState('18'); const [currency, setCurrency] = useState('USD'); const [salt, setSalt] = useState(''); const [cap, setCap] = useState('10000000'); const [uri, setUri] = useState(''); const [initialMint, setInitialMint] = useState(INITIAL_ALLOCATION_MAX.toString()); const [policyIds, setPolicyIds] = useState<Record<string, string>>({});
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (variant === 'stablecoin' && !/^[A-Z]+$/.test(currency)) throw new Error('Stablecoin currency must use uppercase A–Z.');
      ...
      if (initialMintAmount <= 0n) throw new Error('Enter an initial supply greater than zero for your connected wallet.');
      if (initialMintAmount > amount(INITIAL_ALLOCATION_MAX.toString(), d)) throw new Error(`Initial token allocation is limited to ${INITIAL_ALLOCATION_MAX.toString()} tokens.`);
      const capAmount = cap ? amount(cap, d) : null;
      if (capAmount !== null && initialMintAmount > capAmount) throw new Error('Initial supply cannot exceed the supply cap.');
      const active = await client.readContract({ address: ACTIVATION_REGISTRY, abi: activationAbi, functionName: 'isActivated', args: [featureId(variant)] });
      ...
      const initCalls: Hex[] = ROLES.filter((role) => variant === 'asset' || role !== 'OPERATOR_ROLE').map((role) => encodeRoleGrant(role, wallet));
      if (capAmount !== null) initCalls.push(encodeFunctionData({ abi: b20Abi, functionName: 'updateSupplyCap', args: [capAmount] }));
      if (uri) initCalls.push(encodeFunctionData({ abi: b20Abi, functionName: 'updateContractURI', args: [uri] }));
      initCalls.push(encodeFunctionData({ abi: b20Abi, functionName: 'mintWithMemo', args: [wallet, initialMintAmount, memoToBytes32(INITIAL_ALLOCATION_MEMO)] }));
      POLICY_SCOPES.forEach(([scope]) => { const value = policyIds[scope]; if (value && value !== '0') initCalls.push(encodeFunctionData({ abi: b20Abi, functionName: 'updatePolicy', args: [scopeId(scope), BigInt(value)] })); });
      const configured: string[] = [variant === 'asset' ? 'Granted you the issuer roles (mint, burn, pause, metadata, operator)' : 'Granted you the issuer roles (mint, burn, pause, metadata)'];
      ...
      configured.push(`Minted ${formatAmount(initialMintAmount, d)} ${symbol} to your connected wallet with the “${INITIAL_ALLOCATION_MEMO}” memo`);
      const policyCount = POLICY_SCOPES.filter(([scope]) => { const value = policyIds[scope]; return value && value !== '0'; }).length;
      if (policyCount) configured.push(`Applied ${policyCount} initial ${policyCount === 1 ? 'policy' : 'policies'}`);
      const data = encodeFunctionData({ abi: factoryAbi, functionName: 'createB20', args: [variant === 'asset' ? 0 : 1, deploySalt, params, initCalls] });
      const address = await client.readContract({ address: B20_FACTORY, abi: factoryAbi, functionName: 'getB20Address', args: [variant === 'asset' ? 0 : 1, wallet, deploySalt] });
      const hash = await onSend(`Create ${symbol}`, B20_FACTORY, data, 'create_b20');
      if (hash) { await waitForB20Initialization(address); await onCreated({ address, name, symbol, decimals: d, variant, hash, configured }); setSalt(''); }
    } catch (error) { setError(walletErrorMessage(error)); }
    finally { setFinalizing(false); }
  };
  if (created) return <CreatedView created={created} onNavigate={onNavigate} onReset={onReset} />;
  const pending = !!busy || finalizing;
  return <div className="flex flex-col gap-5"><ModuleHeading icon="↗" title="Native Deployment" description="Create a B20 directly through the singleton Factory precompile." /><Card className="bg-white p-5 dark:bg-white/5"><div className="flex gap-2">{(['asset', 'stablecoin'] as const).map((item) => <button key={item} type="button" onClick={() => setVariant(item)} className={cn('rounded-full px-3 py-1.5 text-[12px]', variant === item ? 'bg-base-blue text-white' : 'bg-bds-gray-5 text-bds-gray-60 dark:bg-white/10')}>{item === 'asset' ? 'Asset' : 'Stablecoin'}</button>)}</div><div className="mt-5 grid gap-4 md:grid-cols-2"><Field label="Token name"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="My Token" /></Field><Field label="Symbol"><Input value={symbol} onChange={(e) => setSymbol(e.target.value)} placeholder="MYT" /></Field>{variant === 'asset' ? <Field label="Decimals"><Input value={decimals} onChange={(e) => setDecimals(e.target.value)} inputMode="numeric" /></Field> : <Field label="Currency"><Input value={currency} onChange={(e) => setCurrency(e.target.value.toUpperCase())} placeholder="USD" /></Field>}<Field label="Salt (optional)" help={B20_HELP.salt}><Input value={salt} onChange={(e) => setSalt(e.target.value)} placeholder="Auto-generated when empty" /></Field><Field label="Supply cap (optional)" help={B20_HELP.supplyCap}><Input value={cap} onChange={(e) => setCap(e.target.value)} placeholder="Unlimited" inputMode="decimal" /></Field><Field label="Initial token allocation" hint="Minted with the “Initial deposit” memo · Maximum 100 tokens"><Input value={initialMint} onChange={(e) => setInitialMint(e.target.value)} placeholder="0" inputMode="decimal" /></Field><Field label="Contract URI (optional)"><Input value={uri} onChange={(e) => setUri(e.target.value)} placeholder="https://…" /></Field></div><div className="mt-5"><Text variant="label">Initial policy IDs</Text><Text variant="footnote" tone="muted" className="mt-1">Policy IDs set who can use each token action at launch.</Text><div className="mt-3 grid gap-3 sm:grid-cols-2">{POLICY_SCOPES.map(([scope, label]) => <Field key={scope} label={label}><Input value={policyIds[scope] ?? '0'} onChange={(e) => setPolicyIds((current) => ({ ...current, [scope]: e.target.value }))} inputMode="numeric" /></Field>)}</div></div><div className="mt-5 rounded-xl border border-bds-gray-10 bg-bds-gray-5 p-4 dark:border-white/10 dark:bg-white/5"><div className="flex items-center gap-1.5"><p className="text-[12px] text-bds-gray-50">Deterministic address preview</p><InfoTooltip label="About the deterministic address">{B20_HELP.deterministicAddress}</InfoTooltip></div><p className="mt-1 font-mono text-[13px]">{predicted}</p><p className="mt-3 text-[11px] text-bds-gray-50">Bootstrap grants issuer roles, then configures cap/URI, mints initial supply, and applies restrictive policies.</p></div><ErrorNote message={error} /><Button className="mt-5" onClick={() => void submit()} disabled={pending}>{pending ? 'Creating your token…' : 'Create B20 token'}</Button>{pending ? <p className="mt-3 text-[12px] text-bds-gray-50">Confirm in your wallet, then wait for the token to be mined and initialized. This can take a few seconds.</p> : null}</Card></div>;
}
...
function CreatedView({ created, onNavigate, onReset }: { created: CreatedToken; onNavigate: (module: Module) => void; onReset: () => void }) {
  const nextSteps: Array<{ module: Module; title: string; body: string }> = [
    { module: 'policy', title: 'Inspect policies', body: 'See the token’s policy scopes and check which addresses are authorized.' },
    { module: 'memos', title: 'View memo history', body: 'See your initial allocation memo and send future memo-tagged operations.' },
    ...(created.variant === 'asset' ? [{ module: 'announcements' as Module, title: 'Publish announcements', body: 'Post a disclosure or schedule a UI multiplier update. You hold OPERATOR_ROLE.' }] : []),
  ];
  return (
    <div className="animate-in flex flex-col gap-5">
      <div className="relative flex flex-col items-center gap-3 overflow-hidden rounded-2xl border border-bds-green-20 bg-bds-green-0 px-6 py-8 text-center dark:border-bds-green-80 dark:bg-bds-green-100/20">
        <ConfettiBurst />
        <span className="relative flex h-12 w-12 items-center justify-center rounded-full bg-bds-green-50 text-xl text-white shadow-[0_0_0_8px_rgba(8,173,117,0.12)]" aria-hidden="true">✓</span>
        <Text as="h2" variant="title2">Token created</Text>
        <Text variant="body" tone="muted" className="max-w-md">Your {created.variant} token {created.symbol} is live on Vibenet and fully initialized. Here’s what was set up, and where to go next.</Text>
      </div>
      <div className="grid gap-5 lg:grid-cols-2">
        <Card className="bg-white p-5 dark:bg-white/5">
          <Text variant="label" tone="muted">Token</Text>
          <div className="mt-2 flex flex-wrap items-baseline gap-2"><Text variant="title3">{created.name}</Text><span className="rounded-full bg-bds-gray-5 px-2 py-0.5 text-[11px] capitalize dark:bg-white/10">{created.variant}</span></div>
          <dl className="mt-4 space-y-3 text-[13px]">
            <div className="flex items-center justify-between gap-3"><dt className="text-bds-gray-50">Symbol</dt><dd>{created.symbol}</dd></div>
            <div className="flex items-center justify-between gap-3"><dt className="text-bds-gray-50">Decimals</dt><dd>{created.decimals}</dd></div>
            <div className="flex items-center justify-between gap-3"><dt className="text-bds-gray-50">Initial memo</dt><dd>{INITIAL_ALLOCATION_MEMO}</dd></div>
            <div className="flex items-center justify-between gap-3"><dt className="text-bds-gray-50">Address</dt><dd><CopyableValue value={created.address} display={shortAddress(created.address)} /></dd></div>
          </dl>
          <div className="mt-5 flex flex-wrap gap-4">
            <Link href={`${VIBENET_EXPLORER_PATH}/address/${created.address}`} target="_blank" rel="noreferrer" className="text-[12px] text-base-blue hover:underline">View token on Explorer ↗</Link>
            <Link href={`${VIBENET_EXPLORER_PATH}/tx/${created.hash}`} target="_blank" rel="noreferrer" className="text-[12px] text-base-blue hover:underline">Creation transaction ↗</Link>
          </div>
        </Card>
        <Card className="bg-white p-5 dark:bg-white/5">
          <Text variant="label" tone="muted">What the deployment did</Text>
          <ul className="mt-4 space-y-2.5">{created.configured.map((item) => <li key={item} className="flex items-start gap-2 text-[13px]"><span className="mt-0.5 text-bds-green-60" aria-hidden="true">✓</span><span>{item}</span></li>)}</ul>
          <p className="mt-4 text-[11px] text-bds-gray-50">All applied atomically during initialization, so the token was fully configured in a single transaction.</p>
        </Card>
      </div>
      <section>
        <Text variant="label" tone="muted">What’s next</Text>
        <div className="mt-3 grid gap-3 sm:grid-cols-3">{nextSteps.map((step) => <button key={step.module} type="button" onClick={() => onNavigate(step.module)} className="group flex flex-col rounded-xl border border-bds-gray-10 bg-white p-4 text-left transition-colors hover:border-base-blue dark:border-white/10 dark:bg-white/5"><Text variant="headline">{step.title}</Text><Text variant="footnote" tone="muted" className="mt-1">{step.body}</Text><span className="mt-3 text-[12px] text-base-blue transition-transform group-hover:translate-x-0.5">Go →</span></button>)}</div>
      </section>
      <div className="flex justify-center border-t border-bds-gray-10 pt-5 dark:border-white/10"><Button variant="outline" size="sm" onClick={onReset}>Create another token</Button></div>
    </div>
  );
}

function ModuleHeading({ title, description, action }: { icon?: string; title: string; description: string; action?: React.ReactNode }) { return <section className="flex items-start justify-between gap-3"><div><Text as="h2" variant="title2">{title}</Text><Text variant="body" tone="muted">{description}</Text></div>{action}</section>; }
...
function ErrorNote({ message }: { message: string | null }) { return message ? <p role="alert" className="mt-4 rounded-lg bg-bds-red-0 p-3 text-[13px] text-bds-red-70 dark:bg-bds-red-100/30 dark:text-bds-red-20">{message}</p> : null; }
