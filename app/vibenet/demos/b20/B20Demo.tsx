...
const MODULES: Array<{ value: Module; label: string; icon: string }> = [
  { value: 'policy', label: 'Policy Viewer', icon: '◈' },
  { value: 'memos', label: 'Memos', icon: '▤' },
  { value: 'announcements', label: 'Announcements', icon: '◌' },
  { value: 'deploy', label: 'Native Deployment', icon: '↗' },
];
...
function Input({ value, onChange, placeholder, className, ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input value={value} onChange={onChange} placeholder={placeholder} className={cn('h-10 w-full rounded-lg border border-bds-gray-10 bg-white px-3 text-[13px] outline-none transition-colors placeholder:text-bds-gray-40 focus:border-base-blue dark:border-white/10 dark:bg-white/5 dark:text-white', className)} {...props} />;
}

function Field({ label, children, hint }: { label: string; children: React.ReactNode; hint?: string }) {
  return <label className="flex min-w-0 flex-col gap-1.5"><span className="text-[12px] font-medium text-bds-gray-70 dark:text-bds-gray-20">{label}</span>{children}{hint ? <span className="text-[11px] text-bds-gray-50 dark:text-bds-gray-40">{hint}</span> : null}</label>;
}
...
export function B20Demo() {
  ...
  const inspect = useCallback(async (candidate = tokenAddress) => {
    ...
    try {
      const [isB20, initialized] = await Promise.all([
        client.readContract({ address: B20_FACTORY, abi: factoryAbi, functionName: 'isB20', args: [address] }),
        client.readContract({ address: B20_FACTORY, abi: factoryAbi, functionName: 'isB20Initialized', args: [address] }),
      ]);
  ...
  return (
    <div className="animate-in -mb-20 flex min-h-[calc(100vh-116px)] flex-col gap-5 pb-6 text-black dark:text-white">
...
function SampleAnnouncementViewer({ onDeploy }: { onDeploy: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <ModuleHeading icon="◌" title="Announcement reader" description="Read disclosures published for this sample Asset B20." />
            ...
            <Card className="bg-white p-5 dark:bg-white/5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <Text className="mt-3" variant="headline">{announcement.title}</Text>
...
function AnnouncementModule({ token, tokenAccess, wallet, onDeploy, onSend, busy }: { token: TokenInfo | null; tokenAccess: TokenAccess; wallet: Address | null; onDeploy: () => void; onSend: (label: string, to: Address, data: Hex, action: string) => Promise<boolean>; busy: string | null }) {
  ...
  const loadTemplate = useCallback(() => { setId(`demo-split-${Date.now().toString(36)}`); setDescription('2:1 forward split demonstration'); setUri('https://example.com/disclosures/demo-split'); setMultiplier('2'); setEffectiveAt(futureDatetimeLocal()); }, []);
  useEffect(() => { if (tokenAccess === 'operator' && !id && !description && !effectiveAt) loadTemplate(); }, [description, effectiveAt, id, loadTemplate, tokenAccess]);
  const submit = async () => { if (!token || token.variant !== 'asset') return; try { if (!wallet) throw new Error('Connect the wallet that operates this token first.'); if (!id.trim() || !description.trim() || !effectiveAt) throw new Error('Enter an announcement ID, description, and future effective time.'); const effectiveAtMs = Date.parse(effectiveAt); if (!Number.isFinite(effectiveAtMs) || effectiveAtMs <= Date.now()) throw new Error('Choose a valid effective time in the future.'); const isOperator = await client.readContract({ address: token.address, abi: b20Abi, functionName: 'hasRole', args: [roleId('OPERATOR_ROLE'), wallet] }); if (!isOperator) throw new Error('The connected wallet does not have OPERATOR_ROLE for this token, so it cannot publish announcements.'); const wad = amount(multiplier, 18); const time = BigInt(Math.floor(effectiveAtMs / 1000)); const inner = encodeFunctionData({ abi: assetAbi, functionName: 'setUIMultiplier', args: [wad, time] }); const data = encodeFunctionData({ abi: assetAbi, functionName: 'announce', args: [[inner], id.trim(), description.trim(), uri.trim()] }); try { await client.estimateGas({ account: wallet, to: token.address, data }); } catch (error) { throw new Error(`Announcement simulation failed before opening the wallet: ${walletErrorMessage(error)}. The ID may already be used or a multiplier update may already be scheduled.`); } await onSend('Asset announcement', token.address, data, 'announce'); } catch (error) { alert(walletErrorMessage(error)); } };
  ...
  return <div className="flex flex-col gap-5"><ModuleHeading icon="◌" title="Announcements" description="Publish Asset token actions with an onchain announcement bracket." /><Card className="bg-white p-5 dark:bg-white/5">{!token ? <EmptyToken /> : token.variant !== 'asset' ? <p className="rounded-lg bg-bds-orange-0 p-4 text-[13px] text-bds-orange-70 dark:bg-bds-orange-100/40">Announcements are available on the Asset variant only. Select an Asset B20 token.</p> : <>{tokenAccess !== 'operator' ? <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-bds-blue-20 bg-bds-blue-0 p-4 text-[13px] dark:border-bds-blue-80 dark:bg-bds-blue-100/30"><div><strong>{tokenAccess === 'sample' ? 'Sample token · Read only' : 'This wallet cannot operate the selected token'}</strong><p className="mt-1 text-bds-gray-60 dark:text-bds-gray-30">Use the example as a template, then deploy your own token to sign and publish it.</p></div><Button size="sm" onClick={onDeploy}>Create your own token</Button></div> : <div className="mb-5 rounded-xl bg-bds-green-0 p-3 text-[12px] text-bds-green-70 dark:bg-bds-green-100/30 dark:text-bds-green-20">Your wallet has OPERATOR_ROLE. This example can be published on the selected token.</div>}<div className="mb-4 flex items-center justify-between gap-3"><Text variant="label">Announcement details</Text><Button size="sm" variant="outline" onClick={loadTemplate}>Use example template</Button></div><div className="grid gap-4 md:grid-cols-2"><Field label="Announcement ID"><Input value={id} onChange={(e) => setId(e.target.value)} placeholder="2026-Q3-split" /></Field><Field label="Disclosure URL"><Input value={uri} onChange={(e) => setUri(e.target.value)} placeholder="https://…" /></Field><Field label="Description"><Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="2:1 forward split" /></Field><Field label="Effective at"><Input value={effectiveAt} onChange={(e) => setEffectiveAt(e.target.value)} type="datetime-local" /></Field><Field label="New UI multiplier" hint="1 = unchanged; 2 = 2:1 forward split"><Input value={multiplier} onChange={(e) => setMultiplier(e.target.value)} placeholder="2" inputMode="decimal" /></Field></div><div className="mt-5 rounded-xl border border-bds-gray-10 bg-bds-gray-5 p-4 text-[12px] dark:border-white/10 dark:bg-white/5"><strong>Included call</strong><p className="mt-1 font-mono text-bds-gray-60 dark:text-bds-gray-40">setUIMultiplier(multiplier WAD, effectiveAt)</p><p className="mt-2 text-bds-gray-50">Publishing requires OPERATOR_ROLE on the selected token.</p></div><Button className="mt-5" onClick={() => void submit()} disabled={!!busy || tokenAccess !== 'operator'}>{busy ? 'Waiting for wallet…' : tokenAccess === 'operator' ? 'Publish announcement' : 'Deploy your own token to publish'}</Button></>}</Card></div>;
}

function DeployModule({ wallet, onSend, onCreated, busy }: { wallet: Address | null; onSend: (label: string, to: Address, data: Hex, action: string) => Promise<boolean>; onCreated: (token: RecentToken) => void; busy: string | null }) {
  const [variant, setVariant] = useState<'asset' | 'stablecoin'>('asset'); const [name, setName] = useState(''); const [symbol, setSymbol] = useState(''); const [decimals, setDecimals] = useState('18'); const [currency, setCurrency] = useState('USD'); const [salt, setSalt] = useState('my-b20-token'); const [cap, setCap] = useState(''); const [uri, setUri] = useState(''); const [initialMint, setInitialMint] = useState(''); const [policyIds, setPolicyIds] = useState<Record<string, string>>({});
  const predicted = useMemo(() => wallet ? `${variant === 'asset' ? '0xB200…00' : '0xB200…01'}${shortAddress(saltFor(salt)).slice(2)}` : 'Connect wallet to preview', [salt, variant, wallet]);
  const submit = async () => { if (!wallet) { alert('Connect a wallet first.'); return; } try { if (!name || !symbol) throw new Error('Name and symbol are required.'); if (variant === 'stablecoin' && !/^[A-Z]+$/.test(currency)) throw new Error('Stablecoin currency must use uppercase A–Z.'); const d = variant === 'asset' ? Number(decimals) : 6; if (d < 6 || d > 18) throw new Error('Asset decimals must be between 6 and 18.'); const deploySalt = saltFor(salt); const params = encodeDeploymentParams(variant, name, symbol, wallet, d, currency); const initCalls: Hex[] = ROLES.filter((role) => variant === 'asset' || role !== 'OPERATOR_ROLE').map((role) => encodeRoleGrant(role, wallet)); if (cap) initCalls.push(encodeFunctionData({ abi: b20Abi, functionName: 'updateSupplyCap', args: [amount(cap, d)] })); if (uri) initCalls.push(encodeFunctionData({ abi: b20Abi, functionName: 'updateContractURI', args: [uri] })); if (initialMint) initCalls.push(encodeFunctionData({ abi: b20Abi, functionName: 'mint', args: [wallet, amount(initialMint, d)] })); POLICY_SCOPES.forEach(([scope]) => { const value = policyIds[scope]; if (value && value !== '0') initCalls.push(encodeFunctionData({ abi: b20Abi, functionName: 'updatePolicy', args: [scopeId(scope), BigInt(value)] })); }); const data = encodeFunctionData({ abi: factoryAbi, functionName: 'createB20', args: [variant === 'asset' ? 0 : 1, deploySalt, params, initCalls] }); const address = await client.readContract({ address: B20_FACTORY, abi: factoryAbi, functionName: 'getB20Address', args: [variant === 'asset' ? 0 : 1, wallet, deploySalt] }); if (await onSend(`Create ${symbol}`, B20_FACTORY, data, 'create_b20')) onCreated({ address, name, symbol, decimals: d, variant }); } catch (error) { alert(walletErrorMessage(error)); } };
  return <div className="flex flex-col gap-5"><ModuleHeading icon="↗" title="Native Deployment" description="Create a B20 directly through the singleton Factory precompile." /><Card className="bg-white p-5 dark:bg-white/5"><div className="flex gap-2">{(['asset', 'stablecoin'] as const).map((item) => <button key={item} onClick={() => setVariant(item)} className={cn('rounded-full px-3 py-1.5 text-[12px]', variant === item ? 'bg-base-blue text-white' : 'bg-bds-gray-5 text-bds-gray-60 dark:bg-white/10')}>{item === 'asset' ? 'Asset' : 'Stablecoin'}</button>)}</div><div className="mt-5 grid gap-4 md:grid-cols-2"><Field label="Token name"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="My Token" /></Field><Field label="Symbol"><Input value={symbol} onChange={(e) => setSymbol(e.target.value)} placeholder="MYT" /></Field>{variant === 'asset' ? <Field label="Decimals"><Input value={decimals} onChange={(e) => setDecimals(e.target.value)} inputMode="numeric" /></Field> : <Field label="Currency"><Input value={currency} onChange={(e) => setCurrency(e.target.value.toUpperCase())} placeholder="USD" /></Field>}<Field label="Salt"><Input value={salt} onChange={(e) => setSalt(e.target.value)} placeholder="Unique deployment salt" /></Field><Field label="Supply cap (optional)"><Input value={cap} onChange={(e) => setCap(e.target.value)} placeholder="Unlimited" inputMode="decimal" /></Field><Field label="Initial mint (optional)"><Input value={initialMint} onChange={(e) => setInitialMint(e.target.value)} placeholder="0" inputMode="decimal" /></Field><Field label="Contract URI (optional)"><Input value={uri} onChange={(e) => setUri(e.target.value)} placeholder="https://…" /></Field></div><div className="mt-5"><Text variant="label">Initial policy IDs</Text><div className="mt-3 grid gap-3 sm:grid-cols-2">{POLICY_SCOPES.map(([scope, label]) => <Field key={scope} label={label}><Input value={policyIds[scope] ?? '0'} onChange={(e) => setPolicyIds((current) => ({ ...current, [scope]: e.target.value }))} inputMode="numeric" /></Field>)}</div></div><div className="mt-5 rounded-xl border border-bds-gray-10 bg-bds-gray-5 p-4 dark:border-white/10 dark:bg-white/5"><p className="text-[12px] text-bds-gray-50">Deterministic address preview</p><p className="mt-1 font-mono text-[13px]">{predicted}</p><p className="mt-3 text-[11px] text-bds-gray-50">Bootstrap grants issuer roles, then configures cap/URI, mints initial supply, and applies restrictive policies.</p></div><Button className="mt-5" onClick={() => void submit()} disabled={!!busy}>{busy ? 'Waiting for wallet…' : 'Create B20 token'}</Button></Card></div>;
}

function ModuleHeading({ icon, title, description }: { icon: string; title: string; description: string }) { return <section className="flex items-center gap-3"><span className="text-3xl">{icon}</span><div><Text variant="title2">{title}</Text><Text variant="body" tone="muted">{description}</Text></div></section>; }
