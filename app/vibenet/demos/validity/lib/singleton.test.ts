...
describe('predictSingleton', () => {
  it('is stable across calls and distinct per contract', () => {
    ...
    const addrs = [first.minter, first.factory, first.helper];
    expect(new Set(addrs.map((addr) => addr.toLowerCase())).size).toBe(3);
...
describe('singletonSalt', () => {
  it('keeps one salt per label', () => {
    ...
    expect(new Set(Object.values(SINGLETON_SALTS)).size).toBe(4);
  });
});
