...
async function approveSync(client, parameters) {
  ...
  return {
    ...args,
    ...resolved === undefined ? {} : { decimals: resolved, formatted: formatUnits(args.value, resolved) },
    receipt
  };
}
...
function toWebAuthnAccount(parameters) {
  ...
  return {
    ...
    publicKey,
    async sign({ hash: hash3 }) {
      const { metadata, raw, signature } = await sign4({
        credentialId: id,
        getFn,
        challenge: hash3,
        rpId
...
class ExecutionRevertedError2 extends BaseError2 {
...
var keystoreAddress = "0x81305d4f4976220D2af17E5Dc246848E235600AC";
...
function actorIdFromPublicKey(publicKey) {
  if (typeof publicKey === "string") {
    if (size(publicKey) !== 64)
      throw new BaseError2(`Public key must be 64 bytes (x || y); got ${size(publicKey)} bytes.`);
...
function assertTransaction(transaction) {
  ...
  if (calls) {
    for (const phase of calls)
      for (const call2 of phase)
        if (call2.value && call2.value !== 0n)
...
function toCallsList(calls) {
  return (calls ?? []).map((phase) => phase.map((call2) => [call2.to, call2.data ?? "0x"]));
}
...
function serializeTransaction2(transaction) {
...
// ../viem/src/_esm/eip8130/accounts/toAccount.js
function toAccount2(parameters) {
...
// ../viem/src/_esm/eip8130/accounts/toSmartAccount.js
async function toSmartAccount2(parameters) {
  ...
  const sign5 = parameters.sign ?? (async (hash3) => {
    if (!owner.sign)
  ...
  return toSmartAccount({
    ...
    async decodeCalls(data) {
      ...
      if (result.functionName === "executeBatch")
        return result.args[0].map((call2) => ({
          to: call2.target,
          value: call2.value,
          data: call2.data
        }));
    ...
    async signUserOperation(parameters_) {
      ...
      return concatHex([authenticator, await sign5(userOpHash)]);
    }
  });
}
...
async function estimateGas2(client, parameters) {
  ...
  for (const size5 of [senderAuthSize, payerAuthSize])
    if (size5 !== undefined && (size5 < 0 || size5 > maxAuthSize))
      throw new BaseError2(`auth size ${size5} out of range (0..=${maxAuthSize}).`);
...
function buildAuthBlob(explicit, authenticator, size5) {
  ...
  const dataLength = size5 ?? canonicalAuthDataLength[authenticator.toLowerCase()];
...
async function getTransaction2(client, parameters) {
...
function encodeWalletCalls(parameters) {
  ...
  return calls.map((phase) => {
    const hasValue = phase.some((call2) => call2.value && call2.value !== 0n);
    if (!hasValue)
      return phase.map((call2) => ({ to: call2.to, data: call2.data ?? "0x" }));
    const normalized = phase.map((call2) => ({
      to: call2.to,
      value: call2.value ?? 0n,
      data: call2.data ?? "0x"
    }));
...
async function waitForTransactionReceipt2(client, parameters) {
  ...
  while (Date.now() < deadline) {
    const receipt = await getTransactionReceipt2(client, { hash: hash3 });
    if (receipt !== null)
      return receipt;
    if (validBefore === undefined) {
      try {
        const tx = await getTransaction2(client, { hash: hash3 });
        if (tx.validBefore > 0)
    ...
    switch (permission.type) {
      ...
      case "contract-call": {
        callScopes.push({
          ...
          selectorRules: permission.data.calls.map((call2) => ({
            selector: toSelector(call2)
          }))
        });
        break;
      }
...
function parseCalls(value) {
  const phases = value;
  return phases.map((phase) => phase.map((call2) => {
    const [to, data] = call2;
...
function parseTransaction(serialized) {
  ...
  const fields = fromRlp(sliceHex(serialized, 1), "hex");
...
function toP256Signer(parameters) {
  ...
  return {
    ...
    publicKey: { x, y },
    async sign({ hash: hash3 }) {
      const { r, s } = sign2({ payload: hash3, privateKey });
      return concatHex([
...
async function sendSponsoredCalls(client, parameters) {
  ...
  const terms = parameters.terms ?? await payerClient.getTerms({
    chainId: numberToHex(chainId),
    from: account.address,
    calls: calls.map((call2) => ({ to: call2.to, data: call2.data ?? "0x" })),
...
export {
  ...
  toWebAuthnAccount,
  toTransactionBody,
  toSmartAccount2 as toSmartAccount,
  ...
  signedActorChangesMagic,
  signedAccountChangesTypehash,
  signTransaction3 as signTransaction,
  signAccountChanges,
